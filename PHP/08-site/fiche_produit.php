...
*/
    ...
    $requete = executeRequete("SELECT id_produit, photo, titre FROM produit WHERE id_produit <> :id_produit AND categorie = :categorie ORDER BY RAND() LIMIT 2", array(':id_produit'=> $produit['id_produit'], ':categorie' => $produit['categorie'])); // <> revient à !=


while($suggestion = $requete->fetch(PDO::FETCH_ASSOC)) {
    $aside .= '<div class="col-sm-3">
                <a href="?id_produit='. $suggestion['id_produit'] .'">
                    <img class="img-responsive" src="'. $suggestion['photo'] .'">    
                </a>
