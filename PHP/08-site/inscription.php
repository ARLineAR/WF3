...
$inscritption = false; // varaible qui permet de savoir si le membre est inscrit pour ne pas réafficher le formulaire d'inscrition
...
if(!empty($_POST)) { // si le formulaire est posté
    ...
    if(strlen($_POST['mdp']) < 4 || strlen($_POST['mdp'])> 20) {
        $contenu .='<div class="bg-danger">Le mot de passe doit contenir au moins 2 caractères </div>';
    }
    ...
    // filter_var() permet de valider des formats de chaïnes de caractères pour vérifier qu'il s'agit ici d'un email (on pourrait valider une URL par exemple).
    ...
    /*  Explication de l'expression régulière : 
        ...
        {5} est un quantificateur qui indique qu'il y a 5 caractères autorisés (pas plus pas moins)
    */
    ...
    if(empty($contenu)) {   // si contenu est vide c'est qu'il n' pas d'erreur
        ...
        if($membre->rowCount() > 0) {   // s'il y a des lignes dans le résultat de la requête
            ...
            $contenu  .='<div class=bg-succes">Vous êtes inscrits. <a href="connexion.php">Cliquez ici pour vous conncter</a></div>';
            $inscription = true;    // pour ne plus afficher le formulaire d'inscription

        }   // fin du else if($membre->rowCount() > 0)
...
if (!$inscritption) :   // si membre non-inscrit ($inscritption vaut false), on affiche le formulaire
?>
