...
if(!internauteEstConnecteEtEstAdmin()) {
    header('location:../connexion.php'); // si membre pas admin, alors on le redirigie vers la page de connexion qui est à la racine du sit (en dehors du dossier admin)
    exit();
...
if(isset($_GET['action']) && $_GET['action'] == 'suppression' && isset($_GET['id_produit'])) {
    ...
    $_chemin_photo_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . $produit_a_supprimer['photo']; // chemin du fichier à supprimer

    if(!empty($produit_a_supprimer['photo']) && file_exists($_chemin_photo_a_supprimer)){
        // s'il y a un chemin de photo en base ET que le fichier existe on peut le supprimer
        unlink($_chemin_photo_a_supprimer); // supprime le fichier spécifié
    }

    // Puis suppression du prosuit en BDD :
...
if($_POST) { // équivalent à !empty($_POST) car si $_POST est rempli, il vaut true = formulaire posté
    ...
    //Ici il faudrait vérifier que chaque champ du formulaire est bien rempli (mettre les contrôles)

    $photo_bdd = ''; // la photo subit un traitement spécifique en BDD. Cette variable contiendra son chemein d'accès
    ...
    if(!empty($_FILES['photo']['name'])) {  // si une image a été uploadée, $_FILES est remplie
        ...
        //  On constitue un nom unique pour chaque photo : 
        $nom_photo = $_POST['reference'] . '_' . $_FILES['photo']['name'];

        // On constitue le chemein de la photo enregistrée en BDD :
    ...
    // 4- suite de l'enregistremen en BDD :
...
// 6- Affichage produits dans le back-office :
if(isset($_GET['action']) && $_GET['action'] == 'affichage' || !isset($_GET['action'])) { // si $_GET contient affichage ou que l'on arrive sur la page la première fois ($_GET['action'] n'existe pas)
    ...
    $contenu .= '<table class="table">';
        ...
        $contenu .= '<tr>';
            for($i = 0; $i < $resultat->columnCount(); $i++) {
                ...
                $contenu .= '<th>' . $colonne['name'] . '</th>'; // getColumnMeta() retourne un array contenant notamment l'indice name contnenant le nom de la colonne
        ...
        $contenu .= '</tr>';
            ...
            while ($ligne = $resultat->fetch(PDO::FETCH_ASSOC)){
                    ...
                    // echo '<pre>'; print_r($ligne); echo '</pre>';
                        foreach($ligne as $index => $data) { // $index réceptionne les noms des indices, $data les valeurs des indices
                            if ($index == 'photo') {
...
<form method="post" enctype="multipart/form-data" action=""><!-- "multipart/form-data" permet d'uploader un fichier ou de générer une super globale $_FILES -->
    ...
    <label>Public</label>
    <input type="radio" name="public" value="m"<?php if(isset($produit_actuel['public']) && $produit_actuel['public'] == 'M') echo 'checked'; ?>>Homme
    <input type="radio" name="public" value="f"<?php if(isset($produit_actuel['public']) && $produit_actuel['public'] == 'f') echo 'checked'; ?>>Femme
    <input type="radio" name="public" value="mixte" checked>Mixte<br><br>
    ...
    <?php
        if (isset($produit_actuel['photo'])) {
            echo'<i>Vous uploader une nouvelle photo</i><br>';
            // Afficher la photo actuelle :
            echo '<img src="'. $produit_actuel['photo'] .'" width="90" height="90"><br>';
            // Mettre le chemein de la photo dans un champ caché pour l'enregistrer en base :
            echo '<input type="hidden" name="photo_actuelle" value="'. $produit_actuel['photo'].'">';
            // ce champ renseigne le $_POST['photo_actuelle'] qui va en base quand on soumet le formulaire de modification. Si on ne remplit pas le formulaire ici, le champ photo de la base est remplacé par un vide, qui efface le chemin de la photo
        }
